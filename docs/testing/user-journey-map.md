...
| J32.20 | Os pedidos do cliente (onda 3, bloco 3.1): "Perceber pedido para falar com uma pessoa" e "…para parar de receber mensagens", vistos pela tela | com o Jev ligado no aceite de cada mensagem, as duas aparecem no cartão com o selo "Nova", **observando sozinhas** (R7), sem "Deixar o Jev decidir"; a mensagem que chega pelo WhatsApp só leva ao Jev a pergunta que a regra de hoje NÃO pegou ("PARAR" nunca leva a de parar de receber; "quero falar com um atendente" leva só a de parar de receber; "me deixa em paz", que no turno passa a conversa, não leva nenhuma), numa chamada separada da do clima, e só onde o turno rodaria (o portão do dreno: agente publicado no número da conversa ou roteador ativo nele); o cartão diz "Nos últimos 30 dias, o Jev percebeu 1 pedido de falar com uma pessoa que a regra de hoje não pegou.", com o link para a conversa, e a conversa segue sem passagem, silêncio nem bloqueio; em Execuções a chamada se chama "Perceber pedidos do cliente" | **Spec escrita, NÃO executada nesta rodada** (`tests/e2e/jev-pedidos.spec.ts`, na `SPECS_PARTE_5`, contra o dublê com `DUBLE_JEV_RESPOSTAS`; a regra da rodada proibiu `next build`). **PASS em unit/jsdom**: `lib/ai/decisao/pedidos.test.ts` (cascata com a regra de verdade, denominador, corte 0,9, gravação sem texto), `tests/unit/clima-da-conversa-no-worker.test.ts` (o worker: chamada própria, fatos do turno, clima intacto), `lib/ai/decisao/ponto.test.ts` (a chave só sai com TODAS as tarefas da chamada rodando), `CartaoDoJev.test.tsx` e `app/api/v1/ai/jev/route.test.ts`. **`test:db`**: `tests/invariants/jev-pergunta-so-com-quem-nao-esta-pausado.test.ts` (o agente pausado pela tela — a versão segue publicada — não conta como quem atende) e `tests/invariants/jev-regra-da-rajada-le-o-que-o-turno-le.test.ts` (a regra olha as mensagens sem resposta que o turno lê, a rajada inclusa) |
| J32.21 | "Avisar a equipe" (onda 3, bloco 3.2): o `decidindo` das tarefas em cascata, visto pela tela | no cartão, a tarefa de pedido oferece **"Avisar a equipe"** (nunca "Deixar o Jev decidir"); o diálogo diz o efeito antes de valer ("abre um aviso na Central… nunca passa a conversa sozinho" / "…nunca bloqueia ninguém"); decidindo, o selo é "Avisa a equipe" e "Voltar a só observar" desfaz; a frase natural de um cliente novo abre **UM** aviso na Central (`jev_pedido_de_humano`, `warn`, título e corpo no idioma da organização, **sem a frase do cliente**) com "Abrir a conversa" → `/app/inbox/<id>`; a conversa segue sem dono, sem silêncio e o contato sem bloqueio, no mesmo estado da conversa só observada; depois do clique, o CARTÃO segue "observando" (avisar não é decidir); o aviso fecha quando a conversa é passada a uma pessoa por qualquer caminho, assumida ou encerrada, e o de parar de receber também quando o contato é bloqueado | **Spec escrita, NÃO executada nesta rodada** (os passos novos de `tests/e2e/jev-pedidos.spec.ts`, na `SPECS_PARTE_5`; a regra da rodada proibiu `next build`). **PASS em unit/jsdom**: `lib/ai/decisao/pedidos.test.ts` (um aviso por conversa e pedido, idioma, corte, R3 pelas operações que rodam, Central recusando), `tests/unit/clima-da-conversa-no-worker.test.ts` (pelo worker, pt-BR e es), `CartaoDoJev.test.tsx`, `app/api/v1/ai/jev/route.test.ts`, `lib/ai/inbox-destino.test.ts`. **`test:db`**: `tests/invariants/jev-aviso-na-central.test.ts` (o CHECK aceita os dois kinds; assumir ou encerrar fecha só o aviso da conversa certa; `ai_handling` não fecha), `tests/invariants/jev-aviso-fecha-sozinho-e-e-unico.test.ts` (a passagem do turno, a do clima e a pausa manual fecham o de pessoa; o bloqueio do contato fecha o de parar de receber; um aviso por conversa e pedido pelo índice único, com a deduplicação da migration) e `tests/invariants/jev-pergunta-so-onde-o-dreno-atende.test.ts` (o worker e o dreno respondem igual sobre quem atende; as palavras de passagem de quem pode atender) |
