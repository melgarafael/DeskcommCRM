...
| 29 | 🟠 **`pnpm test:unit` não fechava no Windows, e o controle positivo escondia isso.** O mesmo teste do achado 28 chamava `execFileSync("npx", ...)`: no Windows o `npx` só existe como `npx.cmd`, e o `execFileSync` sem `shell: true` devolve ENOENT (mitigação do CVE-2024-27980). Pior que o vermelho: o controle positivo só exigia `ok === false`, e ENOENT também dá `ok:false` — ele passava **pelo motivo errado**, que é o "medir nada e ficar verde" que o próprio arquivo diz querer evitar | corrigido — invoca `process.execPath` com `--import tsx` (sem `shell: true`, sem barra invertida no script: quem recorta linha é o pai). O controle agora exige a mensagem do validador Zod do env, não um erro qualquer. Provado: 223/223 arquivos e 1891/1891 testes verdes no Windows; a mesma invocação verde em Linux/Node 22 (Docker); e, com a chamada antiga reposta, o controle **reprova** apontando `spawnSync npx ENOENT` |
