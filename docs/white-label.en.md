<!-- traduzido-de: docs/white-label.md@7d57a8428ffd -->
...
⚠️ **The caveat is AI, and it applies to almost every installation.** The sentence above only holds while no foreign AI provider is switched on. Automated customer service sends the conversation to the provider the client connected (Anthropic, OpenAI, Google, DeepSeek or OpenRouter, all outside Brazil). Jev, once the administrator switches it on under AI › Providers, sends every message a customer writes to TypeSafe AI, in the United States, one at a time and without the rest of the conversation, after stripping CPF, phone number and email. Each Jev task has its own purpose — measuring the mood of the conversation, noticing attempts to manipulate the automated service, choosing which agent answers and noticing requests to talk to a person or to stop receiving messages — and all of them use only that one message, on its own. To choose the agent, the intents the company itself registered in the router go along (the company's text, not the customer's). Those flows are international transfers, and the standard clauses apply to them. Jev ships switched off and asks for the administrator's consent before sending anything; the customer-service AI does not.
