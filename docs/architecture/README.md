...
| `banco-de-dados-externo.architecture.json` | banco externo do agente (migration 0233) — 17 peças, 28 arestas, 5 faixas; o cadastro por admin e a leitura por todos, a guarda de rede revalidada a cada pool, e as duas tools SOMENTE-LEITURA com audit sem valores de filtro (a não-ligação declarada: sem allowlist por tabela, D6) |
