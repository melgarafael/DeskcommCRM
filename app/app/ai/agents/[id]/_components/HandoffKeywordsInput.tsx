  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        ...
        <button
          type="button"
          className="rounded-full border border-border/60 px-3 text-xs hover:bg-muted"
          onClick={() => add(draft)}
      ...
      <div className="flex flex-wrap gap-1">
        {DEFAULTS.filter((d) => !value.includes(d)).map((d) => (
          <button
            ...
            onClick={() => add(d)}
            disabled={disabled}
            className="rounded-full border border-dashed border-border/60 px-2 py-0.5 text-xs text-muted-foreground hover:bg-muted"
          >
            + {d}
          </button>
        ))}
