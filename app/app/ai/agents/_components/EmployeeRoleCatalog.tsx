    ...
    <section aria-labelledby="catalogo-funcoes" className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="max-w-2xl">
          <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-primary ring-1 ring-primary/20">
            <Sparkle size={12} weight="fill" aria-hidden /> Equipe pronta para montar
          </div>
          <h2 id="catalogo-funcoes" className="text-xl font-semibold tracking-tight">
            Contrate por função
          </h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            Cada funcionário começa com missão, limites e linguagem da função. Você revisa o
            treinamento, escolhe o número e publica quando estiver pronto.
          </p>
        </div>
      </div>
          ...
          return (
            <div key={department} className="space-y-3">
              <div className="flex items-center gap-3">
                <Badge
                  variant="outline"
                  className={`rounded-full border-0 px-3 py-1 text-[10px] uppercase tracking-[0.16em] ring-1 ${DEPARTMENT_STYLE[department]}`}
                >
                  {department}
                </Badge>
                  ...
                  return (
                    ...
                    >
                      <div className="flex h-full min-h-64 flex-col rounded-[1.15rem] bg-card p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {count > 0 ? (
                            <span className="inline-flex size-8 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-400/20">
                              <Check size={15} weight="bold" aria-label={`${count} contratado`} />
                            </span>
                          ) : (
                        ...
                        <p className="mt-4 text-xs leading-5 text-foreground/80">{role.mission}</p>

                        <div className="mt-auto flex items-end justify-between gap-3 pt-5">
                          <span className="text-xs text-muted-foreground">
                            {count === 0
                              ? "Ainda não contratado"
                              : `${count} ${count === 1 ? "na equipe" : "na equipe"}`}
                          </span>
                          {canWrite ? (
                            <Link
                            ...
                            >
                              {count > 0 ? "Adicionar outro" : "Contratar"}
