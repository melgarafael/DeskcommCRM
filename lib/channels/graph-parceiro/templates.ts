...
import { resolveGraphPartnerCreds } from "./credentials";
...
export const graphPartnerTemplateOps: ChannelTemplateOps = {
  async list({ organizationId, sessionRef }): Promise<ChannelTemplate[]> {
    ...
    let url: string | null = `${c.rootUrl}/v1/${encodeURIComponent(c.wabaId)}/message_templates?limit=100&fields=${fields}`;
  ...
  async remove({ organizationId, sessionRef, name }): Promise<void> {
    const c = await creds({ organizationId, sessionRef });
    const url = `${c.rootUrl}/v1/${encodeURIComponent(c.wabaId)}/message_templates?name=${encodeURIComponent(name)}`;
...
async function postMessageTemplate(
  c: { wabaId: string; token: string; rootUrl: string },
  body: Record<string, unknown>,
): Promise<ChannelTemplate> {
  const url = `${c.rootUrl}/v1/${encodeURIComponent(c.wabaId)}/message_templates`;
  const res = await fetch(url, {
    method: "POST",
