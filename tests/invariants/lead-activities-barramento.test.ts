...
beforeAll(() => {
  ...
  // cpf_hash, os nomes corretos NA ÉPOCA. A 0169 (CPF/CNPJ→NUIT) renomeou as
  // colunas via ALTER TABLE, que não segue para dentro de corpo de função
  // plpgsql (texto opaco); a 0171 é o forward-fix que devolve a função ao
  ...
  psql(readFileSync("supabase/migrations/20260902120000_0171_redact_contact_usa_nuit.sql", "utf8"));
});
