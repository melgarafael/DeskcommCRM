...
describe("ligada — a transição", () => {
  ...
  it("I3b · horário marcado para o mês que vem: 'Cliente desde' é o dia em que se combinou, não o do horário", async () => {
    ...
    const { rows } = await pool.query<{ criado: Date; inicio: Date; agora: Date }>(
      `insert into calendar_appointments (organization_id, title, starts_at, ends_at, contact_id)
       values ($1, 'Futuro', now() + interval '30 days', now() + interval '30 days 1 hour', $2)
       returning created_at as criado, starts_at as inicio, clock_timestamp() as agora`,
      [ORG_A, contato],
    );
    const { criado, inicio, agora } = rows[0]!;
    expect(inicio.getTime(), "controle: o horário é mesmo futuro").toBeGreaterThan(
      agora.getTime(),
    );

    const depois = await lerContato(contato);
    expect(depois.first_service_at?.toISOString()).toBe(criado.toISOString());
    // Compara no mesmo relógio do Postgres. Em Docker Desktop, o relógio da VM
    // pode estar alguns milissegundos à frente do host e `Date.now()` tornava o
    // invariante vermelho mesmo com `first_service_at = created_at` correto.
    expect(depois.first_service_at!.getTime()).toBeLessThanOrEqual(agora.getTime());
