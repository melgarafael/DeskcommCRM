...
async function jaEnviado() {
  ...
  await pool.query(
    `insert into calendar_appointments(id,organization_id,contact_id,conversation_id,owner_user_id,title,
     ...
     values($1,$2,$3,$4,$5,'Reunião',now()+interval '4 days',now()+interval '4 days 1 hour','confirmed',
       ...
       jsonb_build_object('state','sent','generation',gen_random_uuid()::text,'service_boundary',$6::jsonb,
                          'authorized_by',jsonb_build_object('kind','user','id',$5::text)))`,
...
it("⛔ mudar o HORÁRIO de um compromisso já enviado reenfileira a correção", async () => {
  ...
  await pool.query(
    "update calendar_appointments set starts_at=starts_at+interval '1 day' where id=$1",
    [f.id],
  );
...
it("⛔ compromisso CANCELADO não recebe correção", async () => {
  ...
  const f = await jaEnviado();
  await pool.query(
    "update calendar_appointments set status='cancelled',cancelled_at=now(),starts_at=starts_at+interval '1 day' where id=$1",
    [f.id],
  );
...
it("⛔ ANTIRREPETIÇÃO: duas remarcações seguidas deixam UM job vivo", async () => {
  ...
  await pool.query(
    "update calendar_appointments set starts_at=starts_at+interval '2 days' where id=$1",
    [f.id],
  );
