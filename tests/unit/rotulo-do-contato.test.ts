...
describe("a sétima cópia não nasce", () => {
  ...
  const LEITURAS_LEGITIMAS: ReadonlyArray<{ arquivo: string; trecho: string; motivo: string }> = [
    ...
    {
      arquivo: "lib/channels/social/service.ts",
      trecho: "display_name: channel.display_name || channel.type,",
      motivo: "nome da CONEXÃO social recebido da Meta, não de contato",
    },
    {
      arquivo: "lib/channels/social/service.ts",
      trecho: 'local.display_name ?? "Conexão social",',
      motivo: "nome da CONEXÃO social persistida, não de contato",
    },
    {
