...
describe("o que o produto pinta — todo par, toda semente", () => {
  ...
  it("a instrumentação vê os papéis frágeis — inclusive o de stop fixo por tema", () => {
    // O anel de foco usa stop FIXO por tema (600 no claro, 400 no escuro) e não
    // passa por token nenhum — é exatamente a categoria que a régua alcança e
    // que a emissão deixava para trás. Se ele sumir do conjunto medido, os
    // pares acima ficam verdes sem cobri-lo. (Na Sage o claro era 500; na rampa
    // violeta o 500 dá 2,3 contra o branco e o anel subiu para o 600.)
...
describe("o bloco emitido não pode contradizer o globals.css", () => {
  ...
  it("a caminhada de fato ANDA — e a emissão anda junto", () => {
    ...
    // seriam verdes contra o defeito. 11 combos (semente × tema) andam — o mesmo
    ...
    expect(andaram).toBe(11);
  });
});

describe("controle positivo — o produto sem marca não pode se mexer", () => {
  it("o violeta do produto reproduz, pintado, os números do design system", () => {
    // `#7e77f0` é a semente do próprio produto: ela não desloca nada, e os pares
    // pintados têm que dar o que o `globals.css` sempre deu. Se estes números
    // mudarem, o conserto vazou para quem não pediu.
    const cor = corDe("#7e77f0");
