-- 0205 · Quem paga a mídia não via a mídia.
--
-- ─── O buraco que isto fecha ────────────────────────────────────────────────
-- A 0204 fechou o caminho de VOLTA: a venda fechada no CRM vira conversão na
...
--
--  1. O índice único da 0204 é `(organization_id, platform)` — UMA linha por
--     plataforma por organização. Os dois tokens têm ESCOPOS DIFERENTES na Meta
--     (a 0204 quer permissão de escrita no dataset de conversões; esta quer
...
--
--  2. `enabled` é UM interruptor. Ele existe na 0204 para PAUSAR o envio de
...
-- de `calendar_connections`, `channel_sessions` e da 0204 — sem terceiro caminho
...
-- Copiá-lo seria cargo cult. Em 0204 ele pausa um worker que roda sozinho e
...
-- Mesmo desenho de `platform_google_oauth` (0201) e da 0204, pelo mesmo motivo:
...
--
-- Um token `ads_read` é MENOS perigoso que o da 0204 (lê, não escreve), e ainda
...
create table if not exists public.ad_insights_connections (
  ...
  -- Mesmo vocabulário agnóstico da 0204 (`PlataformaDeAnuncio`): a plataforma
...
-- Uma conexão de leitura por plataforma por organização. Mesma lição da 0204 e
