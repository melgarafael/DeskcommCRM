...
as $$
  ...
  regras as (
    ...
    select nullif(btrim(etiqueta.valor #>> '{}'), '') as tag
  ...
  regras_limpo as (
    select r.tag from regras r where r.tag is not null and r.tag <> ''
  ),
  bruto as (
  ...
  select
    ...
    (select count(*) from regras_limpo r
...
as $$
  ...
  from (
    -- `distinct on` pela chave canônica: a primeira ocorrência manda, e é ela
    -- que carrega a substituição. Sem isto, juntar "novo" com "Novo" deixaria
    -- as duas na lista.
    select distinct on (lower(btrim(e.valor)))
           case
    ...
    order by lower(btrim(e.valor)), e.ord
  ) as n
...
begin
  ...
  loop
    ...
    perform public.emit_event(
      p_event_type := 'contact.tags_changed',
      p_entity_kind := 'contact',
      p_entity_id := v_id,
      p_payload := jsonb_build_object(
        'acao', p_acao, 'de', v_tag, 'para', nullif(v_destino, ''), 'caused_by', 'vocabulario_de_tags'
      ),
      p_metadata := jsonb_build_object('caused_by', 'vocabulario_de_tags'),
  ...
  loop
    v_leads := v_leads + 1;
    perform public.emit_event(
      p_event_type := 'lead.tags_changed',
      p_entity_kind := 'lead',
      p_entity_id := v_id,
      p_payload := jsonb_build_object(
        'acao', p_acao, 'de', v_tag, 'para', nullif(v_destino, ''), 'caused_by', 'vocabulario_de_tags'
  ...
  loop
    ...
    perform public.emit_event(
      p_event_type := 'conversation.tags_changed',
  ...
  v_depois := coalesce(
    (
      ...
      from (
        select distinct on (lower(btrim(coalesce(e.valor ->> 'tag', e.valor #>> '{}'))))
               case
        ...
        order by lower(btrim(coalesce(e.valor ->> 'tag', e.valor #>> '{}'))), e.ord
      ) as entrada
  ...
  v_depois := coalesce(
    (
      ...
      from (
        select distinct on (lower(btrim(s.valor #>> '{}')))
               case
                 when v_remover then null
        ...
        where btrim(s.valor #>> '{}') <> ''
        order by lower(btrim(s.valor #>> '{}')), s.ord
      ) as semente
