...
-- ---- ciclo de vida do contexto do agente (migration 0176) ----
...
-- ---- hard reset atômico do contexto (migration 0177) ----
...
-- ---- debounce inbound por organização (migration 0178) ----
...
-- ---- hard reset não é de quem está logado (migration 0179) ----
-- `fn_hard_reset_contact_context` (0177) é DEFINER e ESCREVE, e recebe a
-- organização por ARGUMENTO — não olha membership, porque quem chama já
-- resolveu isso (o único call site usa o client de SERVICE ROLE). O bootstrap
-- do Supabase, porém, grava um `ALTER DEFAULT PRIVILEGES ... TO anon,
-- authenticated` antes de qualquer SQL nosso: sem estes revokes, a função
-- nasce alcançável por RPC do PostgREST com a sessão do browser de QUALQUER
-- tenant. O `revoke ... from public` da 0177 não cobre isso — são duas origens
-- de EXECUTE, cada uma com o seu revoke (lição das 0108/0116).
-- Idempotente: revogar privilégio ausente é no-op.
